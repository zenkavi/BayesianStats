...
Why is the coef of dt_tau NA? Because it is linearly related to 1 or more of the other regressors. Need to further solve algebra to have only one of each term.

$$x_{i}(t+dt) = (1-\frac{dt}{\tau_i}+\frac{{dt}^2}{2{\tau_i}^2}){x_{i}(t)} + \frac{dt}{2\tau_i}+ \Bigg[gN_i(t+dt) + s\phi\Bigg((1-\frac{dt}{\tau_i})x_{i}(t) + \frac{dt}{\tau_i}\big[gN_i(t) + s\phi\big(x_{i}(t)\big) + {I}_{i}(t)\big]\Bigg) + {I}_{i}(t+dt) - gN_i(t) - s\phi\big(x_{i}(t)\big) - {I}_{i}(t)\Bigg]$$
