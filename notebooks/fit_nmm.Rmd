...
```{r}
run_ucr_glm(net_dat_debug)$ucr_task_betas
...
Problems to solve: 
[coding] Why is the first term (coef of stimulated node) different when task reg is specified?
[conceptual] Why is the baseline for the non stim node not at 0?

```{r}
run_ext_glm(net_dat_debug, cur_args_dict)$ext_task_betas
