...
fit$summary()
```
Estimates are almost identical using `nmm_ode2.stan` where the states are not estimated are parameters. But the fitting is not faster either.

```{r eval=FALSE}
mod2 = cmdstan_model("../../stanModels/neuralMass/nmm_ode2.stan")
```
