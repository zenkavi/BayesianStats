...
```{r}
types = c("full", "partial")

for(i in 1:length(methods)){
  for(j in 1:length(types)){
    print(paste0("Stim node is: ", args_dicts[[i*j]]$stim_node))
    print(paste0("Corrected estimates using: ", methods[i], "-", types[j]))
    print(round(run_ext_glm(net_dat, args_dicts[[i*j]], task_reg = cur_args_dict$I[1,])$ext_task_betas, 4))
  }
}
```

Need to fix this before looking at the above

```{r}
dim(net_dat_debug$Enodes)
round(run_ext_glm(net_dat_debug, cur_args_dict, task_reg = cur_args_dict$I[1,])$ext_task_betas, 4)
```
