...
out = data.frame(s=rep(NA,length(sweep_vals)^2), g=rep(NA,length(sweep_vals)^2), mean_stimnodes_end=rep(NA,length(sweep_vals)^2), mean_nonstimnodes_end=rep(NA,length(sweep_vals)^2))
cur_row=1
for(i in 1:length(sweep_vals)){
  for(j in 1:length(sweep_vals)){
    ...
    out[cur_row,] = c(cur_s, cur_g, cur_stimend, cur_nonstimend)
    cur_row = cur_row+1
  }
}
```
