...
First check network structure: confirm that the number of stim nodes and the sum of weights from stim nodes to non stim hub nodes is larger than that for non stim local nodes
...
tmp = data.frame(sum_stim_node_cons = rowSums(big_G$W[11:105, 1:10]), 
           ...
           distance = rowMeans(dist_mat[11:105 , 1:10])) %>%
  mutate(node = 1:n(),
         node = node+10,
