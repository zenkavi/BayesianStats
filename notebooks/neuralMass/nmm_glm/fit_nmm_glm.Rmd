...
data.frame(big_W_net_dat) %>%
  ...
  ggplot(aes(time, value, col=comm, shape=comm))+
  geom_point()+
  theme(legend.title = element_blank())
```

When does activity turn off?

```{r}
sweep_vals = seq(0,1.5,.1)
...
out = data.frame(s=rep(NA,length(sweep_vals)^2), g=rep(NA,length(sweep_vals)^2), min_val=rep(NA,length(sweep_vals)^2))
cur_row=1
for(i in 1:length(sweep_vals)){
  for(j in 1:length(sweep_vals)){
    ...
    cur_min_val = min(sweep_W_net_dat[,dim(sweep_W_net_dat)[2]])
    out[cur_row,] = c(cur_s, cur_g, cur_min_val)
    cur_row = cur_row+1
  }
}
```
