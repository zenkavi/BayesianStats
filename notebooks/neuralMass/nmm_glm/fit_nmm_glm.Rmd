...
Plotting as I did before to visually detect if there is any "bleeding" to non-stim hub nodes. Where has the bleeding gone?
...
How to plot overestimation by distance from stim/hub node?

x = distance (average distance from stimulated nodes)
y = overestimation (ucr - ext)

It doesn't look like there is any difference between the distance from stim nodes to non stim hub nodes compared to the distance from stim nodes to non stim local nodes.

```{r}
#from in columns 
#to in rows
#so i want the stim_node columns for non-stim-node rows

dist_mat = distances(graph_from_adjacency_matrix(big_W_args_dict$W, weighted=T), algorithm = 'johnson')
rowMeans(dist_mat[11:105 , 1:10])
```

What is bleeding? [Especially for UCR] Higher overestimation of task activity to nodes closer (/directly connected to) stimulated nodes.

Since the task regressor used for all nodes in the same
Are the time series of non stim hub nodes more similar to stim nodes than other nodes are to stim nodes?
They should be since they should be more likely to be connected to a stim node

Even before the time series: confirm that the number of stim nodes and the sum of weights from stim nodes to non stim hub nodes is larger than that for non stim local nodes

```{r}
rowSums(big_W_args_dict$W[11:105, 1:10])
```