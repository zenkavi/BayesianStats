...
exp inverse temperature

```{r}
exp(sim_data$simulation$observation)
```
...
```{r}
choice_bin = sim_data$choices
m_bin_data = list(T = TN, choice = choice_bin, outcome = outcome)
```

```{r}
m_bin = stan_model('../stanModels/QLearning_binom.stan')
```

```{r}
fit_nuts_bin = sampling(m_bin, iter=200, chains=4, data=m_bin_data)
```

```{r}
fit_vb_bin = vb(m_bin, data=m_bin_data)
```

```{r}
summary(fit_nuts_bin, pars = c('A', 'tau', 'log_lik'))$summary
```

```{r}
summary(fit_vb_bin, pars = c('A', 'tau', 'log_lik'))$summary
```

```{r}
data.frame(extract(fit_nuts_bin, c("A", "tau"))) %>%
  mutate(alg = "NUTS") %>%
  rbind(data.frame(extract(fit_vb_bin, c("A", "tau"))) %>%
  mutate(alg = "ADVI")) %>%
  gather(key, value, -alg) %>%
  ggplot(aes(x = value, fill=alg))+
  geom_histogram(position="identity", alpha = .5)+
...
```{r}
data.frame(extract(fit_nuts_bin, c('A', 'tau', 'log_lik'))) %>%
  mutate(alg = "NUTS") %>%
  rbind(data.frame(extract(fit_vb_bin, c('A', 'tau', 'log_lik'))) %>%
  mutate(alg = "ADVI"))%>%
  ggplot(aes(x=A, y=tau, color=log_lik))+
  geom_point() +
  facet_grid(~alg)+
