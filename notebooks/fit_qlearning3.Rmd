...
```{r}
f_true = sampling(m_true, iter=1000, chains=4, data=m_data, show_messages=FALSE, verbose=FALSE, refresh= 0)
```

```{r}
f_twoAlphas = sampling(m_twoAlphas, iter=1000, chains=4, data=m_data, show_messages=FALSE, verbose=FALSE, refresh= 0)
```

```{r}
f_diffAlphas = sampling(m_diffAlphas, iter=1000, chains=4, data=m_data, show_messages=FALSE, verbose=FALSE, refresh= 0)
```

```{r}
f_strawMan = sampling(m_strawMan, iter=1000, chains=4, data=m_data, show_messages=FALSE, verbose=FALSE, refresh= 0)
```
...
```{r}
data.frame(p_y_m = c(b_true$logml, b_twoAlphas$logml, b_diffAlphas$logml, b_strawMan$logml),
           model = c("true", "two Alphas", "diff Alphas", "play all")) %>%
  mutate(p_y_m = exp(p_y_m),
         denom = sum(p_y_m),
         p_m_y = p_y_m/denom,
